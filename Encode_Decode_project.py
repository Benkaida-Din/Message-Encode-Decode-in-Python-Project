...
root.resizable(0,0)
root.title('Tech With Din - Message Encode and Decode')
# lables 
Label(root,text = "ENCODE DECODE",font = 'arial 20 bold').pack()
Label(root,text = "Tech With Din",font ='arial 20 bold').pack(side=BOTTOM)
...
def Decode(key,message):
	...
	for i in range(len(message())):
		key_c = key[i % len(key)]
		dec.appedn(chr(256 + ord(message[i] - ord(key_c)) % 256 ))
	return "".join(dec)
def Mode():
	if(mode.get()=="e"):
